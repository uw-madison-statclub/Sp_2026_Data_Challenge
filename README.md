...
Your first task is to help the restaurant owner select `attributes` that successful restaurants share (and determine what successful even means). You must decide exactly which `attributes` would provide the most value, and back up with statistical analysis the ones you determine. 
